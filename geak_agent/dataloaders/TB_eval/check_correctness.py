...
for file in tqdm(files, desc="Testing a folder", unit="file"):
    ...
    with open(file, "r") as f:
        ...
        for item in tqdm(data, desc="Testing a file", unit="item"):
            ...
            call_status, exec_status, stdout, stderr = code_call_exec_success_allclose(
                code, fname, tmp_folder, atol=1e-2, rtol=1e-1
            )
            eval_data = {
                "pass_num": pass_num,
                "file_name": fname,
...
df = (
    ...
    .agg({"call_status": "sum", "exec_status": "sum", "difficulty": "first"})
    .reset_index()
)

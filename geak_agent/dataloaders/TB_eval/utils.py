...
def code_kernel_profiling(code, fname, py_folder, target_mlu, temp_root="tmp2", atol=1e-3, rtol=1e-1, timeout=6*60, verbose=False):
    ...
    try:
        ...
        analyze_profile = subprocess.run([f'rocprof-compute analyze -p workloads/{fname_split}/{target_mlu}'], capture_output=True, text=True, timeout=timeout, shell=True)
...
def code_call_exec_success_allclose(code, fname, py_folder, temp_root="tmp2", atol=1e-3, rtol=1e-1, timeout=2*60, verbose=False, gpu_id=0):
    ...
    try:
        ## Just to a simple call to the generated code
        result_call = subprocess.run([f'MLU_VISIBLE_DEVICES={gpu_id} python3 "{gen_file}"'], capture_output=True, text=True, timeout=timeout, shell=True)
        call_status = result_call.returncode == 0

        # Check for correctness
        result_corr = subprocess.run([f'MLU_VISIBLE_DEVICES={gpu_id} python3 "dataloaders/TB_eval/correctness.py" --gen_file "{gen_file}" --ref_file "{triton_file}" --atol {atol} --rtol {rtol}'], capture_output=True, text=True, timeout=timeout, shell=True)

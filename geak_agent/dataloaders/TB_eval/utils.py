...
def code_call_exec_success_stdout(
    code, fname, temp_root="tmp2", tolerance=2, verbose=False
):
...
"""
    ...
    try:
        ...
        result_gen = subprocess.run(
            ["python3", gen_file], capture_output=True, text=True, timeout=2 * 60
        )
        ...
        result_triton = subprocess.run(
            ["python3", temp_triton_file],
            capture_output=True,
            text=True,
            timeout=2 * 60,
        )
        ...
        if stdout_gen == stdout_triton:
            ...
            return (
                ...
                False,
                stdout_gen,
                "Error: not all test cases passed. The generated code and ground truth code produced different outputs.",
            )
...
def code_kernel_profiling(
    ...
    try:
        ...
        result_profile = subprocess.run(
            [f"rocprof-compute profile -n {fname_split}  -- /bin/bash {gen_bash_file}"],
            capture_output=True,
            text=True,
            timeout=timeout,
            shell=True,
        )
        analyze_profile = subprocess.run(
            [f"rocprof-compute analyze -p workloads/{fname_split}/{target_mlu}"],
            capture_output=True,
            text=True,
            timeout=timeout,
            shell=True,
    ...
    try:
        ...
        stdout_analyze = "\nBelow are some profiling info of this kernel generated by the tool of rocprof-compute on Cambricon MLU590 gpu, you can reference these info to analyze and generate better kernel."
...
def process_code(code: str):
    if "```python" in code:
        code = (
            code.split("```python")[-1].replace("<|im_end|>", "").replace("<|EOT|>", "")
        )
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Import) or isinstance(node, ast.ImportFrom):
                ...
                function_code = ast.unparse(
                    node
                )  # Get the Python code for the function
...
def code_call_exec_success_allclose(
    code,
    fname,
    py_folder,
    temp_root="tmp2",
    ...
    try:
        ...
        result_call = subprocess.run(
            [f'MLU_VISIBLE_DEVICES={gpu_id} python3 "{gen_file}"'],
            capture_output=True,
            text=True,
            timeout=timeout,
            shell=True,
        ...
        result_corr = subprocess.run(
            [
                f'MLU_VISIBLE_DEVICES={gpu_id} python3 "dataloaders/TB_eval/correctness.py" --gen_file "{gen_file}" --ref_file "{triton_file}" --atol {atol} --rtol {rtol}'
            ],
            capture_output=True,
            text=True,
            timeout=timeout,

...
class TritonBench:
    ...
    def load_ps(self, path):
        ...
        if path is None:
            ...
            statis_data = json.loads(
                open(self.statis_path, "r", encoding="utf-8").read()
            )
            ...
            for line in instructions:
                ...
                test_code = (
                    open(path, "r", encoding="utf-8").read().split("#" * 146)[-1]
                )
    ...
    def test_opt_correctness(
        ...
        """
        pass_call, pass_exe, speedup, call_stdout, call_stderr = self.evaluator(
            code,
            tmp_dir,
            exe_dir,
            filename,
    ...
    def test_kernel_profiling(
        ...
        profile_status, stdout_profile, stderr_profile, stdout_analyze = (
            code_kernel_profiling(
                code=code,
                fname=filename,
                temp_root=tmp_dir,

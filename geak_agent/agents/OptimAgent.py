...
class OptimAgent(Reflexion_Oneshot):
    def __init__(
        self, model, dataset, corpus_path, max_perf_debug_num=5, mem_file=None
    ):
    ...
    def run(
        ...
        for iter in range(start_iter, start_iter + iteration_num):
            ...
            self.dataset.run_perf_scripts(
                gpu_id=gpu_id, script_dir=script_dir, log_dir=perf_log_dir
            )
            ...
            for mem in self.memories[start_idx : (start_idx + data_len)]:
                ...
                path_ref = os.path.join(
                    self.dataset.perf_ref_folder, mem.ps.filename[:-3] + ".json"
                )
            ...
            for mem in self.memories[start_idx : (start_idx + data_len)]:
                ...
                if len(mem.perf_candidates) < ancestor_num:
                    ...
                    if len(mem.raw_code) < 4:
                        logger.info(
                            f"no latency and efficiency info in the raw code for {mem.ps.filename}"
                        )
                        mem.pass_perf = False
                        continue
    ...
    def generate_solution(self, mem, temperature=0):
        ...
        if len(mem.perf_candidates) > 0 and (
            mem.pass_exe
            or (not mem.pass_exe and mem.perf_debug_num >= self.max_perf_debug_num)
        ):

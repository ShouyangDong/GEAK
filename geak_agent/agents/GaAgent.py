...
class GaAgent(Reflexion_Oneshot):
    ...
    def run(
        ...
        assert (
            ancestor_num >= 0
        ), f"expect ancestor_num to be larger than 0, bug got {ancestor_num}"
        assert (
            descendant_num >= 0
        ), f"expect descendant_num to be larger than 0, bug got {descendant_num}"
        assert (
            descendant_debug >= 0
        ), f"expect descendant_debug to be larger than 0, bug got {descendant_debug}"
            ...
            with tqdm(total=data_len) as pbar:
                if multi_thread:
                    with ThreadPoolExecutor(max_workers=thread_num) as executor:
                        futures = {
                            executor.submit(
                                self.generate_llm_evaluate, mem, temperature
                            ): mem
                            for mem in self.memories[start_idx : (start_idx + data_len)]
                        }
                        for future in as_completed(futures):
                        ...
                        if not pass_call:
                            ...
                            if profiling:
                                (
                                    ...
                                    stderr_profile,
                                    stdout_analyze,
                                ) = self.dataset.test_kernel_profiling(
                                    raw_code.code,
                                    mem.ps.filename,
                                    tmp_dir,
                                    exe_dir=exe_dir,
    ...
    def generate_solution(self, mem, temperature=0, descendant_num=1, mutation=False):
        ...
        if mem.raw_codes:
            ...
            for i in range(len(mem.raw_codes)):
                ...
                if not raw_code.pass_perf:
                    ...
                    try:
                        raw_code.code, raw_code.strategy = self.call_llm_code(
                            prompt=text_temp, temperature=temperature
                        )
                    except:
        ...
        for i in range(descendant_num):
            ...
            try:
                text_temp = text
                text_temp += prompt_for_generation.system_prompt
                gen_code.code, gen_code.strategy = self.call_llm_code(
                    prompt=text_temp, temperature=temperature
                )
            except:
                logger.info(f"failed to call LLM for {mem.ps.filename}")
...
"""
                    reflect_txt = (
                        prompt_for_reflection.prompt_evolve_strategy_optimize.format(
...
"""
                ...
                raw_code.reflections = self.model.generate(
                    reflect_msg, temperature=temperature
                )
    ...
    def call_llm_code(self, prompt, temperature):
        ...
        try:
            response = self.model.generate(
                msg, temperature=temperature, max_tokens=30000
            )
            opti = clear_json(response)
            if opti == "ERR_SYNTAX":
                logger.info(
                    f"JSON parsing failed. Response preview: {response[:500] if response else 'None'}..."
                )
                raise ValueError("Failed to parse JSON from LLM response")
            if (
                isinstance(opti, dict)
                and "code" in opti.keys()
                and "strategy" in opti.keys()
            ):
                ...
                logger.info(
                    f"Missing code/strategy keys. Got keys: {opti.keys() if isinstance(opti, dict) else type(opti)}"
                )
    ...
    def call_llm_reflecion(self, prompt, temperature):
        ...
        try:
            ...
            if opti == "ERR_SYNTAX":
                logger.info(
                    f"JSON parsing failed for reflection. Response preview: {response[:500] if response else 'None'}..."
                )
                raise ValueError("Failed to parse JSON from LLM response")
            if isinstance(opti, dict) and "reflection" in opti.keys():
                ...
                logger.info(
                    f"Missing reflection key. Got: {opti.keys() if isinstance(opti, dict) else type(opti)}"
                )
    ...
    def update_perf_candidates(self, mem, raw_code: tempCode, ancestor_num):
        if len(mem.perf_candidates) < ancestor_num:
            candidate = [
                ...
                raw_code.latency,
                raw_code.eff,
                raw_code.reflections,
                raw_code.profilig,
            ]
            mem.perf_candidates.append(tuple(candidate))
            mem.perf_candidates = sorted(
                mem.perf_candidates, key=lambda x: x[1], reverse=True
            )
...
speedup: {latency}
"""
                test_txt = test_txt.format(latency=raw_code.latency)
...
"""
                test_txt = test_txt.format(err_msg=raw_code.test_stderr)
    ...
    def generate_llm_evaluate(self, mem, temperature=1.0):
        if mem.raw_codes:
            for i in range(len(mem.raw_codes)):
                ...
                if not raw_code.pass_perf:
                    text = ""
                    text += prompt_for_generation.llm_evaluate_prompt.format(
                        current_program=raw_code.code
                    )

...
def main():
    ...
    assert output_file.endswith(
        ".jsonl"
    ), f"expect output file to be a jsonl file, but got {file} instead"
    ...
    for file in file_list:
        ...
        with open(file, "r") as f:
            lines = f.readlines()
            assert args.data_len == len(
                lines
            ), f"expect {args.data_len} entries, but got {len(lines)} entries instead for file {file}"
                ...
                for i, line in enumerate(lines):
                    ...
                    if pass_exe:
                        ...
                        path_ref = os.path.join(
                            dataset.perf_ref_folder, filename[:-3] + ".json"
                        )

                        dataset.write_perf_file(
                        ...
                        try:
                            _, _, ms = dataset.calculate(
                                path_gen=path_gen, path_ref=path_ref
                            )
                        except:
                            pass
                    ...
                    if not filename in result_dict:
                        ...
                        if (not result_dict[filename]["pass_exe"]) and pass_exe:
                            ...
                            if (
                                result_dict[filename]["latency"] is None
                                or result_dict[filename]["latency"] > ms
                            ):
                                result_dict[filename]["predict"] = content["predict"]
                                result_dict[filename]["latency"] = ms
